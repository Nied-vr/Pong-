...
def get_config(config_type):
    if current_profile:
        ...
        config = ref.get()
        if config:
            print(f"Configuración para {config_type}: {config}")
        else:
            print(f"No se encontró configuración para {config_type}")
        return config
    else:
        print("No hay perfil actual")
    return {}

def run_pong_classic():
    config = get_config('classic')
    print(config)
    ball_speed = config.get('ball_speed', BALL_SPEED) if config else BALL_SPEED
