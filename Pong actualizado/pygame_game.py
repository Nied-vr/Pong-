import pygame
from game_logic import update_high_score
...
def run_pong_classic():
    config = get_config('classic')
...
def run_pong_speed():
    ...
    initial_ball_speed = config.get('initial_ball_speed', BALL_SPEED)

    def game_loop():
        pygame.init()
        screen = pygame.display.set_mode((WIDTH, HEIGHT))
        pygame.display.set_caption("Pong Velocidad")

        left_pad_pos = HEIGHT // 2 - PAD_HEIGHT // 2
        right_pad_pos = HEIGHT // 2 - PAD_HEIGHT // 2
        ball_pos = [WIDTH // 2, HEIGHT // 2]
        ball_vel = [initial_ball_speed, initial_ball_speed]
        ...
        while running:
            ...
            if ball_pos[0] < 0 or ball_pos[0] > WIDTH:
                if ball_pos[0] < 0:
                    ...
                    score_left += 1
                ball_pos = [WIDTH // 2, HEIGHT // 2]
                ball_vel = [initial_ball_speed, initial_ball_speed] if ball_pos[0] < 0 else [-initial_ball_speed, -initial_ball_speed]
            ...
            if ball_vel[0] < MAX_BALL_SPEED and ball_vel[0] > -MAX_BALL_SPEED:
                ball_vel[0] += 0.01 if ball_vel[0] > 0 else -0.01
            if ball_vel[1] < MAX_BALL_SPEED and ball_vel[1] > -MAX_BALL_SPEED:
                ball_vel[1] += 0.01 if ball_vel[1] > 0 else -0.01

            paddle_speed = max(PADDLE_SPEED, abs(ball_vel[0] / 2))
